...
VERIFY_SCRIPT="${SCRIPT_DIR}/m307-multi-channel-orchestration-depth.sh"
...
report_dir="${tmp_dir}/artifacts/multi-channel-orchestration-depth"
report_path="${report_dir}/verification-report.json"
pass_log="${tmp_dir}/pass.log"

set +e
TAU_M307_MOCK_MODE="1" \
TAU_M307_REPORT_DIR="${report_dir}" \
...
assert_equals "m307_multi_channel_orchestration_depth" "$(jq -r '.suite_id' "${report_path}")" "suite id"
...
assert_contains "$(cat "${pass_log}")" "m307 multi-channel orchestration depth verification passed" "pass output"
...
set +e
TAU_M307_MOCK_MODE="1" \
TAU_M307_MOCK_FAIL_PATTERN="c5_04_whatsapp_webhook_invalid_signature" \
TAU_M307_REPORT_DIR="${report_dir}" \
...
assert_equals "fail" "$(jq -r '.steps[] | select(.id == "c5_04_whatsapp_webhook_invalid_signature") | .status' "${report_path}")" "failed step status"
assert_contains "$(cat "${tmp_dir}/fail.log")" "m307 multi-channel orchestration depth verification failed" "fail output"

tmp_report="${tmp_dir}/report-tampered.json"
jq ' .steps |= map(select(.id != "c5_08_media_attachment_handling")) ' "${report_path}" > "${tmp_report}"
mv "${tmp_report}" "${report_path}"

set +e
missing_output="$(
  TAU_M307_VERIFY_ONLY="1" \
  TAU_M307_REPORT_DIR="${report_dir}" \
  "${VERIFY_SCRIPT}" 2>&1
)"
missing_rc=$?
...
if [[ "${missing_output}" == *"m307 multi-channel orchestration depth verification passed"* ]]; then
  echo "assertion failed (missing markers): verify-only must not report pass" >&2
  exit 1
fi

echo "m307-multi-channel-orchestration-depth verification tests passed"
