...
VERIFY_SCRIPT="${SCRIPT_DIR}/m309-auth-credential-lifecycle-depth.sh"
...
report_dir="${tmp_dir}/artifacts/auth-credential-lifecycle-depth"
report_path="${report_dir}/verification-report.json"
pass_log="${tmp_dir}/pass.log"

set +e
TAU_M309_MOCK_MODE="1" \
TAU_M309_REPORT_DIR="${report_dir}" \
...
assert_equals "m309_auth_credential_lifecycle_depth" "$(jq -r '.suite_id' "${report_path}")" "suite id"
...
assert_contains "$(cat "${pass_log}")" "m309 auth credential lifecycle depth verification passed" "pass output"
...
set +e
TAU_M309_MOCK_MODE="1" \
TAU_M309_MOCK_FAIL_PATTERN="integration_auth_set_status_rotate_revoke_lifecycle" \
TAU_M309_REPORT_DIR="${report_dir}" \
...
assert_equals "fail" "$(jq -r '.steps[] | select(.id == "integration_auth_set_status_rotate_revoke_lifecycle") | .status' "${report_path}")" "failed step status"
assert_contains "$(cat "${tmp_dir}/fail.log")" "m309 auth credential lifecycle depth verification failed" "fail output"

tmp_report="${tmp_dir}/report-tampered.json"
jq ' .steps |= map(select(.id != "resolve_secret_rejects_revoked_secret")) ' "${report_path}" > "${tmp_report}"
mv "${tmp_report}" "${report_path}"

set +e
missing_output="$(
  TAU_M309_VERIFY_ONLY="1" \
  TAU_M309_REPORT_DIR="${report_dir}" \
  "${VERIFY_SCRIPT}" 2>&1
)"
missing_rc=$?
...
if [[ "${missing_output}" == *"m309 auth credential lifecycle depth verification passed"* ]]; then
  echo "assertion failed (missing markers): verify-only must not report pass" >&2
  exit 1
fi

echo "m309-auth-credential-lifecycle-depth verification tests passed"
