...
VERIFY_SCRIPT="${SCRIPT_DIR}/m317-tui-scenario-expansion-depth.sh"
...
report_dir="${tmp_dir}/artifacts/tui-scenario-expansion-depth"
report_path="${report_dir}/verification-report.json"
pass_log="${tmp_dir}/pass.log"

set +e
TAU_M317_MOCK_MODE="1" \
TAU_M317_REPORT_DIR="${report_dir}" \
...
assert_equals "m317_tui_scenario_expansion_depth" "$(jq -r '.suite_id' "${report_path}")" "suite id"
...
assert_contains "$(cat "${pass_log}")" "m317 tui scenario expansion depth verification passed" "pass output"
...
set +e
TAU_M317_MOCK_MODE="1" \
TAU_M317_MOCK_FAIL_PATTERN="tui_parse_shell_profile_missing_value_spec" \
TAU_M317_REPORT_DIR="${report_dir}" \
...
assert_equals "fail" "$(jq -r '.steps[] | select(.id == "tui_parse_shell_profile_missing_value_spec") | .status' "${report_path}")" "failed step status"
assert_contains "$(cat "${tmp_dir}/fail.log")" "m317 tui scenario expansion depth verification failed" "fail output"

tmp_report="${tmp_dir}/report-tampered.json"
jq ' .steps |= map(select(.id != "quickstart_tui_shell_live_watch_docs_contract")) ' "${report_path}" > "${tmp_report}"
mv "${tmp_report}" "${report_path}"

set +e
missing_output="$(
  TAU_M317_VERIFY_ONLY="1" \
  TAU_M317_REPORT_DIR="${report_dir}" \
  "${VERIFY_SCRIPT}" 2>&1
)"
missing_rc=$?
...
if [[ "${missing_output}" == *"m317 tui scenario expansion depth verification passed"* ]]; then
  echo "assertion failed (missing markers): verify-only must not report pass" >&2
  exit 1
fi

echo "m317-tui-scenario-expansion-depth verification tests passed"
