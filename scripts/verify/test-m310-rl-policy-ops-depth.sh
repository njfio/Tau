...
VERIFY_SCRIPT="${SCRIPT_DIR}/m310-rl-policy-ops-depth.sh"
...
report_dir="${tmp_dir}/artifacts/rl-policy-ops-depth"
report_path="${report_dir}/verification-report.json"
pass_log="${tmp_dir}/pass.log"

set +e
TAU_M310_MOCK_MODE="1" \
TAU_M310_REPORT_DIR="${report_dir}" \
...
assert_equals "m310_rl_policy_ops_depth" "$(jq -r '.suite_id' "${report_path}")" "suite id"
...
assert_contains "$(cat "${pass_log}")" "m310 rl policy operations depth verification passed" "pass output"
...
set +e
TAU_M310_MOCK_MODE="1" \
TAU_M310_MOCK_FAIL_PATTERN="rl_rollback_required_gate_integration" \
TAU_M310_REPORT_DIR="${report_dir}" \
...
assert_equals "fail" "$(jq -r '.steps[] | select(.id == "rl_rollback_required_gate_integration") | .status' "${report_path}")" "failed step status"
assert_contains "$(cat "${tmp_dir}/fail.log")" "m310 rl policy operations depth verification failed" "fail output"

tmp_report="${tmp_dir}/report-tampered.json"
jq ' .steps |= map(select(.id != "runtime_promotion_gate_audit_regression")) ' "${report_path}" > "${tmp_report}"
mv "${tmp_report}" "${report_path}"

set +e
missing_output="$(
  TAU_M310_VERIFY_ONLY="1" \
  TAU_M310_REPORT_DIR="${report_dir}" \
  "${VERIFY_SCRIPT}" 2>&1
)"
missing_rc=$?
...
if [[ "${missing_output}" == *"m310 rl policy operations depth verification passed"* ]]; then
  echo "assertion failed (missing markers): verify-only must not report pass" >&2
  exit 1
fi

echo "m310-rl-policy-ops-depth verification tests passed"
