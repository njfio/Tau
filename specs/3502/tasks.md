# Tasks: Issue #3502 - M311 deterministic TUI operator workflow depth gate

Status: Implemented

## Ordered Tasks
1. [x] T1 (RED, Conformance/Regression): add
   `scripts/verify/test-m311-tui-operator-workflow-depth.sh` with
   required-step and report-schema assertions; run pre-implementation to
   capture failure.
2. [x] T2 (GREEN, Implementation): add
   `scripts/verify/m311-tui-operator-workflow-depth.sh` with deterministic TUI
   selector map and JSON report generation.
3. [x] T3 (GREEN, Contract): enforce verify-only fail-closed required-step
   inventory checks.
4. [x] T4 (GREEN, Docs): update README TUI gap links with M311 entrypoint.
...
| Functional | ✅ | `CARGO_TARGET_DIR=target-fast bash scripts/verify/m311-tui-operator-workflow-depth.sh` |  |
| Conformance | ✅ | `bash scripts/verify/test-m311-tui-operator-workflow-depth.sh` |  |
| Integration | ✅ | M311 selector map executes deterministic shell/shell-live/artifact-diagnostic contracts |  |
| Fuzz | N/A |  | No new parser/input surface introduced |
| Mutation | N/A |  | Verification-shell/docs scope, non-critical mutation gate |
| Regression | ✅ | M311 contract pass/fail/tamper verify-only fail-closed checks |  |
