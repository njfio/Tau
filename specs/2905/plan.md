...
5. Render a bounded graph-backed node preview on zero-result pages so operators can inspect memory summaries plus node/relation metadata before jumping to Memory Graph.
...
- Zero-result Memory Explorer pages render a bounded graph-backed preview with memory summaries, node ids, types, importance, connected relation counts/samples, and graph detail links.
