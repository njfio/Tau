...
5. Render a bounded graph-backed node preview on zero-result pages so operators can inspect memory summaries plus node/relation metadata, distinguish a not-run search from a no-match search, see the search-result count and graph-preview availability as separate scope metrics, see the preview's own bounded count/limit proof, follow sample relations into Memory Graph, return to the originating preview row context, see the returned row marked as selected, see the resolved detail row marked independently from preview-return recovery state, recover when the returned memory is outside the bounded preview, distinguish missing return ids from filtered-out return ids, and see an explicit Memory Graph not-found detail state when a recovery `detail_memory_id` cannot be resolved.
...
- The graph-backed preview section exposes visible bounded-preview count/limit proof in addition to the Memory Scope summary.
...
- Memory Explorer graph-backed preview rows expose additive `data-detail-selected`, `data-detail-selected-memory-id`, `data-memory-graph-preview-detail-badge`, and selected-detail copy when a resolved `detail_memory_id` matches a row, independent from `preview_memory_id` recovery state.
