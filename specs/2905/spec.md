...
- Distinguish missing `preview_memory_id` values from filtered-out values and real out-of-preview nodes.
...
- When Memory Explorer is opened with a resolved `detail_memory_id`, mark the matching graph-backed preview row as the selected detail even if `preview_memory_id` is missing or failed recovery.
...
when `/ops/memory` renders,
then the Memory Scope summary and empty-state row expose deterministic search-result, graph-preview, graph node/edge counts and graph state markers, the empty-state copy states whether search is not active or no matching search rows were found, a bounded graph-backed node preview renders visible count/limit proof, memory summaries with node metadata plus navigable relation samples, Memory Graph detail return links preserve the originating preview memory and row anchor when available, the returned preview row is visibly and deterministically marked as selected, the row matching a resolved `detail_memory_id` is visibly and deterministically marked as the selected detail, a real out-of-preview `preview_memory_id` renders a visible explanation plus recovery link, missing and filtered-out `preview_memory_id` values render distinct recovery states without implying the same cause, and the Memory Graph recovery route renders an explicit not-found detail state when the requested `detail_memory_id` cannot be resolved.
...
| C-03a | AC-3a | Functional/Integration | zero search rows with graph rows available | render memory route without a query, render a no-match query, follow a preview node/relation into graph detail, then return with `preview_memory_id`, open with a resolved `detail_memory_id`, or recover through Memory Graph with a missing `detail_memory_id` | scope summary labels the count as search results, shows graph-preview count/limit plus graph node/edge counts, graph state, `not-run` versus no-match search state, bounded graph-backed node preview shows visible count/limit proof, summaries plus relation sample links, graph detail return links include `preview_memory_id` plus the preview row anchor, the returned preview row carries selected markers plus visible selected copy, a resolved detail row carries selected-detail markers plus visible selected-detail copy, real out-of-preview return state includes an explanation plus Memory Graph recovery link, missing return state uses not-found recovery markers/copy, filtered-out return state uses filtered recovery markers/copy, and the Memory Graph recovery route preserves the requested id in a not-found detail state |
