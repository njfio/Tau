...
when `/ops/memory` renders,
then the Memory Scope summary and empty-state row expose deterministic graph node/edge counts and graph state markers, the empty-state copy states that only search rows are empty, and a bounded graph-backed node preview renders memory summaries with node metadata.
...
| C-03a | AC-3a | Functional/Integration | zero search rows with graph rows available | render memory route | scope summary and empty row show graph node/edge counts, graph state, graph-aware copy, and bounded graph-backed node preview with summaries |
