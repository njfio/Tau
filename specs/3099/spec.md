...
- Preserve selected/requested `detail_memory_id` context when pan actions update graph state.
...
when shell HTML is produced,
then pan state is clamped to bounds, left/right/up/down links encode clamped next values,
and selected/requested memory detail context remains encoded.
...
| C-02 | AC-2 | Integration | graph route with `graph_pan_x`/`graph_pan_y` and `detail_memory_id` query | render route | state and directional pan links reflect clamped next states while preserving detail context |
