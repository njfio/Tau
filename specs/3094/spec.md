...
- Preserve selected/requested `detail_memory_id` context when zoom actions update graph state.
...
when shell HTML is produced,
then zoom level is clamped to bounds, zoom-in/out links encode clamped next values,
...
| C-02 | AC-2 | Integration | graph route with `graph_zoom` and `detail_memory_id` query | render route | level and zoom action links reflect clamped next states while preserving detail context |
