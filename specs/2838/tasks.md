...
## Ordered Tasks
1. [ ] T1 (RED): add conformance tests for `/ops/sessions` panel/list/row/empty-state SSR markers in UI and gateway.
2. [ ] T2 (GREEN): implement sessions explorer snapshot/render mapping in UI + gateway.
3. [ ] T3 (REGRESSION): run phase 1N chat selector suites and ops route marker regressions.
4. [ ] T4 (VERIFY): run fmt/clippy/tests/guardrails/mutation and set spec status `Implemented`.
...
- Verify:
  ...
  - `cargo mutants --in-diff /tmp/mutants_2838.diff -p tau-dashboard-ui -p tau-gateway`
