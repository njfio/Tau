# Tasks: Issue #3510 - M313 deterministic E2E core scenario depth gate

Status: Implemented

## Ordered Tasks
1. [x] T1 (RED, Conformance/Regression): add
   `scripts/verify/test-m313-e2e-core-scenario-depth.sh` with required-step and
   report-schema assertions; run pre-implementation to capture failure.
2. [x] T2 (GREEN, Implementation): add
   `scripts/verify/m313-e2e-core-scenario-depth.sh` with deterministic E2E
   selector map and JSON report generation.
3. [x] T3 (GREEN, Contract): enforce verify-only fail-closed required-step
   inventory checks.
4. [x] T4 (GREEN, Docs): update README integration/gap links with M313
   entrypoint.
...
| Functional | ✅ | `CARGO_TARGET_DIR=target-fast bash scripts/verify/m313-e2e-core-scenario-depth.sh` |  |
| Conformance | ✅ | `bash scripts/verify/test-m313-e2e-core-scenario-depth.sh` |  |
| Integration | ✅ | M313 selector map executes deterministic integration-package + gateway lifecycle/session/http/ws/auth contracts |  |
| Fuzz | N/A |  | No new parser/input surface introduced |
| Mutation | N/A |  | Verification-shell/docs scope, non-critical mutation gate |
| Regression | ✅ | M313 contract pass/fail/tamper verify-only fail-closed checks |  |
