...
9. [x] T9 (REGRESSION): target skip-to-main at the visible login shell on
   `/ops/login`.
...
- Conformance: C-01..C-12 mapped in crate/gateway tests.
...
- RED: Live Browser on
  `/ops/login?theme=dark&sidebar=expanded&session=default` showed
  `id=tau-ops-skip-to-main` targeting `#tau-ops-protected-shell` while the
  protected shell had `aria-hidden=true`.
- RED: `RUST_MIN_STACK=16777216 cargo test -p tau-dashboard-ui regression_spec_2786_login_skip_link_targets_visible_login_shell -- --nocapture`
  failed before the skip link target became route-aware.
- GREEN: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-dashboard-ui regression_spec_2786_login_skip_link_targets_visible_login_shell -- --nocapture`
  passed.
- REGRESSION: `RUST_MIN_STACK=16777216 cargo test -p tau-dashboard-ui 2786 -- --nocapture`
  passed (7 tests).
- REGRESSION: `RUST_MIN_STACK=16777216 cargo test -p tau-gateway ops_login -- --nocapture`
...
- LIVE: Rebuilt `tau-coding-agent` running on `127.0.0.1:8795` reported
  ...
  `service=running`; Browser verified `id=tau-ops-skip-to-main` targeting
  `#tau-ops-login-shell`, login shell `aria-hidden=false`, protected shell
  `aria-hidden=true`, and no visible credential input on `/ops/login`.
- RED: Live Browser on
  `/ops/login?theme=dark&sidebar=expanded&session=default` exposed
  `Continue` as a button and the DOM showed
  `id=tau-ops-login-submit role=button` even though the control navigates to
  `/ops`.
- RED: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-dashboard-ui regression_spec_2786_c03_none_login_continue_links_to_ops_shell -- --nocapture`
  failed before the button role was removed from the Continue anchor.
- GREEN: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-dashboard-ui regression_spec_2786_c03_none_login_continue_links_to_ops_shell -- --nocapture`
  passed.
...
- LIVE: Rebuilt `tau-coding-agent` running on `127.0.0.1:8795` reported
  `auth.mode=localhost-dev`, `model=gpt-5.3-codex`, and
  `service=running`; Browser verified Continue appears as one link and zero
...
- LIVE: HTTP proof showed `/ops/login` response size dropped from 233430 bytes
  ...
  route panel ids.
- RED: Live Browser on
  `/ops/login?theme=dark&sidebar=expanded&session=default` still exposed the
  full protected navigation rail before Continue, including Command Center,
  Agent Fleet, Chat, Sessions, Memory, Mission Harness, and Deploy.
- RED: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-dashboard-ui regression_spec_2786_login_route_prunes_visible_protected_navigation -- --nocapture`
...
- REGRESSION: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-gateway ops_auth_navigation -- --test-threads=1 --nocapture`
  passed 9 tests.
- REGRESSION: `RUST_MIN_STACK=16777216 CARGO_INCREMENTAL=0 cargo test -p tau-dashboard-ui -- --nocapture`
  passed 208 tests plus doc tests.
- STATIC: `cargo fmt --check --package tau-dashboard-ui --package tau-gateway`,
  `git diff --check`, and
...
- LIVE: Rebuilt `tau-coding-agent` running on `127.0.0.1:8795` reported
  `auth.mode=localhost-dev`, `model=gpt-5.3-codex`, and
  `service=running`; Browser verified `/ops/login` showed only Home, Operator
  Login, and Continue in the interactive tree, while HTTP proof showed
  `data-nav-scope=login`, protected nav rows hidden, breadcrumb Home pointing
  to `/ops/login`, and Continue still targeting `/ops`.
