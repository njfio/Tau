...
## Ordered Tasks
1. [ ] T1 (RED): add failing tests for v2 keyed envelope roundtrip, legacy v1 compatibility, and tamper rejection.
2. [ ] T2 (GREEN): declare `aes-gcm` and `redb` dependencies in approved manifests.
3. [ ] T3 (GREEN): implement AES-256-GCM keyed encryption for new payloads with legacy v1 decrypt fallback.
4. [ ] T4 (REGRESSION): run scoped tau-provider/provider-auth regression tests and fix drift.
5. [ ] T5 (VERIFY): run fmt/clippy/tests and capture evidence.
6. [ ] T6 (DOC): update G20 roadmap row with `#2774` evidence and mark spec implemented.
