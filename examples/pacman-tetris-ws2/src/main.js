...
const BOARD_X = 28;
const BOARD_Y = 78;
const PANEL_X = BOARD_X + COLS * CELL_SIZE + 32;
const GAME_WIDTH = PANEL_X + 250;
const GAME_HEIGHT = BOARD_Y + ROWS * CELL_SIZE + 34;
const BG_COLOR = 0x07101b;
const GRID_COLOR = 0x0f1b30;
const PANEL_COLOR = 0x091221;
const RAIL_COLOR = 0x63e3ff;
const ACCENT_COLOR = 0xffb347;
const DANGER_COLOR = 0xff6b74;
const TEXT_COLOR = "#f6f3dc";
const MUTED_COLOR = "#8ea6bf";
const PACMAN_STEP_MS = 145;
const GHOST_STEP_MS = 205;
const POWER_MODE_MS = 5200;
const GHOST_RESPAWN_MS = 3200;
...
const TETROMINOES = [
  { name: "I", color: 0x4fe7ff, matrix: [[1, 1, 1, 1]] },
  { name: "O", color: 0xffd44f, matrix: [[1, 1], [1, 1]] },
  { name: "T", color: 0xd381ff, matrix: [[0, 1, 0], [1, 1, 1]] },
  { name: "S", color: 0x61ec7c, matrix: [[0, 1, 1], [1, 1, 0]] },
  { name: "Z", color: 0xff6f89, matrix: [[1, 1, 0], [0, 1, 1]] },
  { name: "J", color: 0x6ea5ff, matrix: [[1, 0, 0], [1, 1, 1]] },
  { name: "L", color: 0xffb168, matrix: [[0, 0, 1], [1, 1, 1]] },
];
...
function topPressure(board, rows = 5) {
  let occupied = 0;
...
class PacmanTetrisWarpShiftScene extends Phaser.Scene {
  constructor() {
    super("pacman-tetris-ws");
  }
  ...
  create() {
    ...
    this.titleText = this.add.text(PANEL_X, 28, "PACMAN\nTETRIS WS", {
    ...
    this.tagText = this.add.text(PANEL_X, 102, "WARP SHIFT", {
    ...
    this.statsText = this.add.text(PANEL_X, 132, "", {
      fontFamily: '"Chakra Petch", "Trebuchet MS", sans-serif',
      fontSize: "19px",
      color: TEXT_COLOR,
      lineSpacing: 7,
    ...
    this.nextLabel = this.add.text(PANEL_X, 330, "NEXT DROP", {
      fontFamily: '"Chakra Petch", "Trebuchet MS", sans-serif',
      fontSize: "14px",
      fontStyle: "700",
      color: "#63e3ff",
    ...
    this.flavorText = this.add.text(PANEL_X, 620, "", {
      fontFamily: '"Chakra Petch", "Trebuchet MS", sans-serif',
      fontSize: "15px",
      color: "#ffcf8f",
      lineSpacing: 6,
  ...
  resetState() {
    ...
    this.powerPellets = new Set([
      ...
      cellKey(COLS - 2, 3),
      cellKey(1, ROWS - 4),
      cellKey(COLS - 2, ROWS - 4),
    ]);

    this.pellets = Array.from({ length: ROWS }, (_, y) =>
      Array.from({ length: COLS }, () => y >= 2 && y <= ROWS - 2),
    );
    ...
    this.ghosts = [
      this.createGhost(1, 2, 0xff6875),
      this.createGhost(Math.floor(COLS / 2), 2, 0x68a8ff),
      this.createGhost(COLS - 2, 2, 0x71f28a),
    ];

    this.pellets[this.pacman.y][this.pacman.x] = false;
    for (const ghost of this.ghosts) {
      this.pellets[ghost.y][ghost.x] = false;
    }
  ...
  stepPacman(time) {
    ...
    if (
      this.canEnterCell(
        this.pacman.x + desired.x,
        this.pacman.y + desired.y,
        true,
      )
    ) {
  ...
  collectPellet(time) {
    ...
    if (this.powerPellets.has(cellKey(this.pacman.x, this.pacman.y))) {
      ...
      this.lastEvent = "Power mode live.";
      this.cameras.main.flash(120, 255, 247, 170);
    } else {
      this.score += 10;
    }
  ...
  remainingPellets() {
    return this.pellets.flat().filter(Boolean).length;
  }
  ...
  tryRotatePiece() {
    ...
    for (const kick of [0, -1, 1, -2, 2]) {
      if (
        this.canPlace(rotated, this.activePiece.x + kick, this.activePiece.y)
      ) {
  ...
  lockPiece(time) {
    ...
    if (cleared.length > 0) {
      ...
      this.score += [0, 160, 380, 720, 1100][cleared.length] ?? 1400;
      this.ghostFreezeUntil = time + 1000 + cleared.length * 260;
      if (this.isPowered(time)) {
        this.pacman.poweredUntil += 600;
      }
      this.lastEvent =
        cleared.length === 1
          ? "Single row burst."
          : `${cleared.length}-row burst.`;
      this.cameras.main.flash(120, 255, 190, 95);
      this.cameras.main.shake(130, 0.005 + cleared.length * 0.0015);
    } else {
      this.lastEvent = `Locked ${this.activePiece.name}-piece.`;
      this.cameras.main.shake(60, 0.0018);
    }

    if (topPressure(this.board) >= 4) {
      this.cameras.main.flash(70, 255, 107, 116);
      this.lastEvent = "Roof pressure rising.";
    }
  ...
  checkEncounters(time) {
    for (const ghost of this.ghosts) {
      ...
      if (this.isPowered(time)) {
        ...
        ghost.y = ghost.spawnY;
        this.score += 240;
        this.lastEvent = "Ghost eaten.";
        this.cameras.main.flash(90, 135, 186, 255);
      } else {
  ...
  refreshHud() {
    const roof = topPressure(this.board);
    const powerLabel = this.isPowered(performance.now()) ? "HOT" : "COLD";
    const ghostLabel =
    ...
    this.statsText.setText(
      [
        ...
        `Left     ${this.remainingPellets().toString().padStart(5, " ")}`,
        `Bursts   ${this.rowBursts.toString().padStart(5, " ")}`,
        `Power    ${powerLabel}`,
        `Ghosts   ${ghostLabel}`,
        `Roof     ${roof}/5`,
      ].join("\n"),
    ...
    this.flavorText.setText(
      this.gameOver
        ? "The cabinet took the round.\nPress R to restart."
        : this.isPowered(performance.now())
          ? "Power mode is live.\nFlip the chase."
          : `${this.lastEvent}\nLine clears calm the board.`,
    );
  }

  getFallInterval() {
    return Math.max(130, 700 - (this.level - 1) * 48);
  }
  ...
  draw(time) {
    ...
    const pulse = 0.6 + (Math.sin(time / 160) + 1) * 0.15;
    const roof = topPressure(this.board);
    const powered = this.isPowered(time);

    this.boardGraphics.fillStyle(PANEL_COLOR, 0.96);
    ...
    this.boardGraphics.lineStyle(
      4,
      roof >= 4 ? DANGER_COLOR : powered ? ACCENT_COLOR : RAIL_COLOR,
      0.88,
    );
    ...
    for (let y = 0; y < ROWS; y += 1) {
      for (let x = 0; x < COLS; x += 1) {
        ...
        this.boardGraphics.fillStyle(GRID_COLOR, 0.92);
        this.boardGraphics.fillRoundedRect(
          pixelX + 2,
          pixelY + 2,
          CELL_SIZE - 4,
        ...
        if (
          ...
          const powerCell = this.powerPellets.has(cellKey(x, y));
          this.boardGraphics.fillStyle(
            powerCell ? ACCENT_COLOR : 0xf6f3dc,
            powerCell ? pulse : 0.82,
          );
          this.boardGraphics.fillCircle(
            pixelX + CELL_SIZE / 2,
            pixelY + CELL_SIZE / 2,
            powerCell ? 5 + Math.sin(time / 120) * 0.8 : 2.5,
          );
        }
    ...
    for (const cell of this.activeCells()) {
      this.drawBlock(
        this.boardGraphics,
        BOARD_X + cell.x * CELL_SIZE,
        BOARD_Y + cell.y * CELL_SIZE,
  ...
  drawBlock(graphics, pixelX, pixelY, color, alpha) {
    ...
    graphics.fillRoundedRect(pixelX + 4, pixelY + 6, CELL_SIZE - 8, CELL_SIZE - 8, 6);
    graphics.fillStyle(color, alpha);
    graphics.fillRoundedRect(pixelX + 3, pixelY + 3, CELL_SIZE - 6, CELL_SIZE - 6, 6);
    graphics.fillStyle(0xffffff, 0.18 * alpha);
    graphics.fillRoundedRect(pixelX + 6, pixelY + 6, CELL_SIZE - 12, 5, 4);
  }
  ...
  drawPacman(time) {
    ...
    const mouth = 0.15 + (Math.sin(time / 90) + 1) * 0.08;
    const angle = this.pacman.dir.angle ?? 0;

    if (this.isPowered(time)) {
      this.boardGraphics.fillStyle(0xfff1c2, 0.14);
      this.boardGraphics.fillCircle(
        centerX,
        centerY,
        radius + 7 + Math.sin(time / 120) * 2,
      );
    }

    this.boardGraphics.fillStyle(0xffd34b, 1);
  ...
  drawGhosts(time) {
    for (const ghost of this.ghosts) {
      ...
      const frightened = this.isPowered(time);
      const color = frightened ? 0x5a84ff : ghost.color;

      this.boardGraphics.fillStyle(color, 0.96);
      this.boardGraphics.fillCircle(
        pixelX + CELL_SIZE * 0.35,
        pixelY + CELL_SIZE * 0.42,
        CELL_SIZE * 0.2,
      );
      this.boardGraphics.fillCircle(
        pixelX + CELL_SIZE * 0.65,
        pixelY + CELL_SIZE * 0.42,
        CELL_SIZE * 0.2,
      );
      this.boardGraphics.fillRoundedRect(
      ...
      this.boardGraphics.fillCircle(pixelX + CELL_SIZE * 0.4, pixelY + CELL_SIZE * 0.47, 4);
      this.boardGraphics.fillCircle(pixelX + CELL_SIZE * 0.6, pixelY + CELL_SIZE * 0.47, 4);
      this.boardGraphics.fillStyle(frightened ? 0xfff6d3 : 0x091221, 1);
      this.boardGraphics.fillCircle(pixelX + CELL_SIZE * 0.42, pixelY + CELL_SIZE * 0.5, 1.7);
      this.boardGraphics.fillCircle(pixelX + CELL_SIZE * 0.62, pixelY + CELL_SIZE * 0.5, 1.7);
    }
  }

  drawNextPiece() {
    this.previewGraphics.fillStyle(PANEL_COLOR, 0.96);
    this.previewGraphics.fillRoundedRect(PANEL_X, 356, 182, 92, 16);
    this.previewGraphics.lineStyle(2, 0x26476b, 0.9);
    this.previewGraphics.strokeRoundedRect(PANEL_X, 356, 182, 92, 16);

    const matrix = this.nextPiece.matrix;
    const offsetX = PANEL_X + 26;
    const offsetY = 384 + Math.max(0, (2 - matrix.length) * 8);
    ...
    for (let y = 0; y < matrix.length; y += 1) {
      for (let x = 0; x < matrix[y].length; x += 1) {
        ...
        this.previewGraphics.fillStyle(this.nextPiece.color, 0.96);
        this.previewGraphics.fillRoundedRect(
          offsetX + x * 24,
          offsetY + y * 24,
          20,
          20,
  ...
  drawOverlay() {
    ...
    this.overlayGraphics.fillStyle(0x04070d, 0.66);
...
const game = new Phaser.Game({
  ...
  scene: [PacmanTetrisWarpShiftScene],
  render: {
    pixelArt: false,
    antialias: true,
  },
...
window.__PACMAN_TETRIS_WS__ = game;
