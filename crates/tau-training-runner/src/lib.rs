...
impl TrainingRunner {
    ...
    ///
    /// Returns an error if `config.validate()` rejects the retry-backoff
    /// or other runner settings. Prefer propagating the error rather than
    /// unwrapping so malformed config surfaces as a readable message at
