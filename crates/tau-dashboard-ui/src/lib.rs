...
mod agent_canvas_panel;
mod chat_markdown;
mod deploy_panel;

pub(crate) use agent_canvas_panel::render_tau_ops_chat_agent_canvas;
pub(crate) use chat_markdown::{
    contains_markdown_contract_syntax, extract_assistant_stream_tokens,
    extract_first_fenced_code_block, parse_chat_markdown_blocks, render_chat_markdown_blocks,
};
pub(crate) use deploy_panel::render_tau_ops_deploy_process_lifecycle;
...
fn ops_shell_context_href(
    ...
    format!(
        "{path}?theme={}&sidebar={}&session={session_key}",
        theme.as_str(),
...
pub fn render_tau_ops_dashboard_shell_with_context(context: TauOpsDashboardShellContext) -> String {
    ...
    let nav_agent_fleet_href = ops_shell_context_href(
        TauOpsDashboardRoute::Agents.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_agent_detail_href = ops_shell_context_href(
        TauOpsDashboardRoute::AgentDetail.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_chat_href = ops_shell_context_href(
        TauOpsDashboardRoute::Chat.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_sessions_href = ops_shell_context_href(
        TauOpsDashboardRoute::Sessions.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_memory_href = ops_shell_context_href(
        TauOpsDashboardRoute::Memory.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    ...
    let nav_channels_href = ops_shell_context_href(
        TauOpsDashboardRoute::Channels.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    ...
    let nav_training_href = ops_shell_context_href(
        TauOpsDashboardRoute::Training.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_safety_href = ops_shell_context_href(
        TauOpsDashboardRoute::Safety.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_diagnostics_href = ops_shell_context_href(
        TauOpsDashboardRoute::Diagnostics.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_deploy_href = ops_shell_context_href(
        TauOpsDashboardRoute::Deploy.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    let nav_login_href = ops_shell_context_href(
        TauOpsDashboardRoute::Login.shell_path(),
        context.theme,
        context.sidebar_state,
        shell_nav_session_key.as_str(),
    );
    ...
    let memory_graph_detail_memory_type_label = if memory_graph_detail_memory_type.trim().is_empty()
    {
        "unknown".to_string()
    } else {
        memory_graph_detail_memory_type.clone()
    };
    let memory_graph_detail_relation_count_panel_attr =
        memory_detail_relation_count_panel_attr.clone();
    let memory_graph_detail_relation_count = memory_detail_relation_count.clone();
    let memory_graph_detail_graph_relation_count = memory_detail_graph_relation_count.clone();
    let memory_graph_detail_graph_relation_count_panel_attr =
        memory_detail_graph_relation_count.clone();
    let memory_graph_detail_selected_entry_label =
        if memory_graph_detail_selected_entry_id.trim().is_empty() {
            "none".to_string()
        } else {
            memory_graph_detail_selected_entry_id.clone()
        };
    let memory_graph_detail_relation_scope_summary = format!(
        "Stored detail relations: {memory_graph_detail_relation_count}; graph connections in this scope: {memory_graph_detail_graph_relation_count}"
    );
    ...
    let memory_graph_detail_relations_view = if memory_detail_graph_relation_edges.is_empty() {
        leptos::either::Either::Left(view! {
            <li id="tau-ops-memory-graph-detail-relations-empty-state" data-empty-state="true">
                No graph connections in this scope.
            </li>
        })
    } else {
        leptos::either::Either::Right(
            memory_detail_graph_relation_edges
                ...
                .enumerate()
                .map(|(index, edge)| {
                    let row_id = format!("tau-ops-memory-graph-detail-relation-row-{index}");
                    let is_outgoing = edge.source_memory_id.as_str()
                        == memory_graph_detail_selected_entry_id.as_str();
                    let direction = if is_outgoing { "outgoing" } else { "incoming" };
                    let connected_memory_id = if is_outgoing {
                        edge.target_memory_id.clone()
                    } else {
    ...
    let chat_latest_assistant_content = latest_assistant_row
        .map(|(_, row)| row.content.clone())
        .unwrap_or_default();
    let chat_latest_assistant_markdown_blocks =
    ...
    let shell = view! {
        ...
        >
            <style id="tau-ops-dashboard-base-style">
                ...
                #tau-ops-chat-token-counter-details[open] {
                    display: grid;
                    gap: 8px;
                }
                ...
                #tau-ops-chat-token-counter-details > summary {
                    display: flex;
                    min-height: 34px;
                    align-items: center;
                ...
                #tau-ops-chat-token-counter-details > summary::-webkit-details-marker {
                    display: none;
                }
                ...
                #tau-ops-chat-new-session-status[data-new-session-status="created"] {
                    border-color: #2f7653;
                    background: #0f251f;
                }
                #tau-ops-chat-new-session-status[data-new-session-status="empty-key"] {
                    border-color: #7c3a3a;
                    background: #2a1418;
                }
                #tau-ops-chat-new-session-status h3,
                #tau-ops-chat-new-session-status p {
                    margin: 0;
                }
                #tau-ops-chat-new-session-status h3 {
                    color: #9bb6c2;
                ...
                #tau-ops-chat-latest-assistant-content,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] {
                    display: grid;
                    gap: 8px;
                    min-width: 0;
                    color: #dbe8ef;
                ...
                #tau-ops-chat-latest-assistant-content p,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] p {
                    margin: 0;
                    overflow-wrap: anywhere;
                }
                #tau-ops-chat-latest-assistant-content h4,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] h4 {
                    margin: 3px 0 0;
                    color: #9bd7b6;
                    font-size: .74rem;
                    font-weight: 800;
                ...
                #tau-ops-chat-latest-assistant-content ul,
                #tau-ops-chat-latest-assistant-content ol,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] ul,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] ol {
                    display: grid;
                    gap: 6px;
                    margin: 0;
                    padding-left: 1.2rem;
                ...
                #tau-ops-chat-latest-assistant-content li,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] li {
                    padding-left: .15rem;
                    overflow-wrap: anywhere;
                }
                #tau-ops-chat-latest-assistant-content strong,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] strong {
                    color: #91f0b9;
                    font-weight: 800;
                }
                #tau-ops-chat-latest-assistant-content code,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] code {
                ...
                #tau-ops-chat-latest-assistant-content pre,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] pre {
                    max-width: 100%;
                    margin: 0;
                    border: 1px solid #254c5c;
                    border-radius: 6px;
                ...
                #tau-ops-chat-latest-assistant-content table,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] table {
                    width: 100%;
                    border-collapse: collapse;
                    overflow-wrap: anywhere;
                    font-size: .74rem;
                }
                #tau-ops-chat-latest-assistant-content th,
                #tau-ops-chat-latest-assistant-content td,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] th,
                #tau-ops-chat-transcript [data-markdown-rendered="true"] td {
                    border: 1px solid #254c5c;
                    padding: 5px 6px;
                    text-align: left;
                    vertical-align: top;
                    ...
                    >
                        ...
                        >
                            ...
                            <form
                                ...
                                data-submit-pending-state="idle"
                            >
                                <input
                            ...
                            <script
                                ...
                                data-pending-submit-state="true"
                            >
                                r#"
                                (function () {
                                    function installChatComposeShortcuts() {
                                        ...
                                        var sendButton = document.getElementById("tau-ops-chat-send-button");
                                        ...
                                        form.setAttribute("data-submit-pending-state", "idle");
                                        ...
                                        function resetPendingState() {
                                            ...
                                            if (sendButton) {
                                                ...
                                                sendButton.setAttribute("data-submit-pending", "false");
                                        ...
                                        function markSubmitPending() {
                                            ...
                                            form.setAttribute("data-submit-pending-state", "submitting");
                                            input.setAttribute("readonly", "readonly");
                                            input.setAttribute("aria-busy", "true");
                                            if (sendButton) {
                                                sendButton.disabled = true;
                                                sendButton.setAttribute("aria-disabled", "true");
                                                sendButton.setAttribute("data-submit-pending", "true");
                                                sendButton.textContent = "Sending...";
                                            }
                                            var status = document.getElementById("tau-ops-chat-send-status");
                                            var message = document.getElementById("tau-ops-chat-send-status-message");
                                            if (status) {
                                                status.setAttribute("data-chat-send-status", "submitting");
                                                status.setAttribute("data-chat-send-status-visible", "true");
                                                status.setAttribute("aria-hidden", "false");
                                                status.hidden = false;
                                            }
                                            if (message) {
                                                message.textContent = "Sending message to Tau...";
                                            }
                                        }
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <h4>Assistant</h4>
                                        <article
                                            id="tau-ops-chat-latest-assistant-content"
                                            data-markdown-rendered="true"
                                            data-markdown-block-count=chat_latest_assistant_markdown_block_count
                                        >
                                            {render_chat_markdown_blocks(
                                                chat_latest_assistant_markdown_blocks,
                                                Some("tau-ops-chat-latest-code-block".to_string()),
                                            )}
                                        </article>
                                    </section>
                                </article>
                            ...
                            {render_tau_ops_chat_agent_canvas(
                                chat_agent_canvas_status,
                                chat_agent_canvas_loaded.to_string(),
                                chat_agent_canvas_artifact_path,
                                chat_agent_canvas_srcdoc,
                                chat_agent_canvas_srcdoc_bytes,
                                chat_agent_canvas_placeholder_hidden.to_string(),
                                chat_agent_canvas_loaded_bool,
                            )}
                            <ul
                                    ...
                                    .map(|(index, message_row)| {
                                        ...
                                        if message_row.role == "tool" {
                                            ...
                                            if markdown_contract || code_block_contract.is_some() {
                                                ...
                                                let markdown_blocks =
                                                    parse_chat_markdown_blocks(&message_row.content);
                                                let markdown_block_count =
                                                    markdown_blocks.len().to_string();
                                                view! {
                                                    <li
                                                        id=row_id
                                                    ...
                                                    >
                                                        <article
                                                            id=markdown_card_id
                                                            data-markdown-rendered="true"
                                                            data-markdown-block-count=markdown_block_count
                                                        >
                                                            {render_chat_markdown_blocks(
                                                                markdown_blocks,
                                                                Some(code_block_id),
                            ...
                            <details
                                id="tau-ops-chat-token-counter-details"
                                data-secondary-token-counter="true"
                                data-collapsed-by-default="true"
                        ...
                        >
                            ...
                            <section
                                ...
                                data-graph-relation-count=memory_graph_detail_graph_relation_count_panel_attr
                            >
                                <dl
                                    id="tau-ops-memory-graph-detail-proof"
                                    data-selected-memory-id=memory_graph_detail_selected_entry_id.clone()
                                    data-memory-type=memory_graph_detail_memory_type_label.clone()
                                    data-stored-relation-count=memory_graph_detail_relation_count.clone()
                                    data-graph-relation-count=memory_graph_detail_graph_relation_count.clone()
                                >
                                    <div>
                                        <dt>Selected ID</dt>
                                        <dd>{memory_graph_detail_selected_entry_label}</dd>
                                    </div>
                                    <div>
                                        <dt>Type</dt>
                                        <dd>{memory_graph_detail_memory_type_label.clone()}</dd>
                                    </div>
                                    <div>
                                        <dt>Stored Relations</dt>
                                        <dd>{memory_graph_detail_relation_count.clone()}</dd>
                                    </div>
                                    <div>
                                        <dt>Graph Relations</dt>
                                        <dd>{memory_graph_detail_graph_relation_count.clone()}</dd>
                                    </div>
                                </dl>
                                <p
                                ...
                                <p
                                    id="tau-ops-memory-graph-detail-relation-scope"
                                    data-stored-relation-count=memory_graph_detail_relation_count
                                    data-graph-relation-count=memory_graph_detail_graph_relation_count.clone()
                                >
                                    {memory_graph_detail_relation_scope_summary}
                                </p>
                                <ul
                                    id="tau-ops-memory-graph-detail-relations"
                                    data-selected-memory-id=memory_graph_detail_selected_entry_id.clone()
                                    data-graph-relation-count=memory_graph_detail_graph_relation_count
                                >
                                    {memory_graph_detail_relations_view}
                                </ul>
                            </section>
                        </section>
                        <section
